...
class Select extends BaseComponent{
    ...
    fixDropdownPosition(){
        const veiwportWidth = window.document.documentElement.clientWidth;
        const halfViewportWidth = veiwportWidth / 2;
        const {width,x} = this.buttonElement.getBoundingClientRect();
        const buttonCenterX = x + width / 2;
        const isButtonOnViewportSide = buttonCenterX < halfViewportWidth;

        this.dropdownElement.classList.toggle(this.stateClasses.isOnTheLeftSide, isButtonOnViewportSide);
        this.dropdownElement.classList.toggle(this.stateClasses.isOnTheRightSide, !isButtonOnViewportSide);
